...
export default function ProjectCard({ name, description, teamsCnt, placesCnt }: ProjectCardProps) {
  ...
  return (
    <div className="flex flex-col w-full min-h-64 px-6 py-6 shadow-md inset-shadow-xs rounded-xl bg-[#FBFAFF] border border-gray-200 hover:shadow-lg transition-shadow">
      <div className="pb-4 border-b border-[#000150]/20 mb-4">
        <h2 className="text-[20px] text-[#000150] font-semibold mb-3 line-clamp-2">
          {truncateText(name, 60)}
        </h2>
        <p className="text-[16px] text-[#000150] mb-1 line-clamp-3">{truncateText(description, 120)}</p>
      </div>

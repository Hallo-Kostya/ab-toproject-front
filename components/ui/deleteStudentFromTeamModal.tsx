...
interface DeleteStudentFromTeamModalProps {
  isOpen: boolean;
  onClose: () => void;
  studentId: string;
  studentName: string;
...
export default function DeleteStudentFromTeamModal({ 
  isOpen, 
  onClose, 
  studentId, 
  studentName, 
  onConfirm 
}: DeleteStudentFromTeamModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="p-6 bg-white rounded-[24px]">
        ...
        <h2 className="text-2xl text-[#000150] font-bold mb-2 text-center">Удалить студента из команды</h2>
        <p className="mb-6 text-center text-gray-600">
          Вы точно хотите удалить студента <span className="font-semibold text-[#000150]">{studentName}</span> из команды?
        </p>
